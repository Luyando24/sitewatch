  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in-up">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link to="/dashboard/reports" className="btn-secondary text-xs">
          📋 Full Report
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard label="Total Sites" value={exec?.total_sites || 0} icon="🗼" sub={`${exec?.sites_up} online`} />
        <KPICard label="Overall Uptime" value={`${exec?.overall_uptime_pct || 0}%`} color="emerald" icon="📶" sub={`${exec?.sites_down} down`} />
        <KPICard label="Active Alerts" value={exec?.active_alerts || 0} color={exec?.critical_alerts > 0 ? "red" : "blue"} icon="🔔" sub={`${exec?.critical_alerts} critical`} />
        <KPICard label="Monthly Revenue" value={`KES ${((exec?.monthly_revenue || 0) / 1000).toFixed(0)}K`} icon="💰" sub={`${exec?.total_tenants} tenants`} />
      </div>
